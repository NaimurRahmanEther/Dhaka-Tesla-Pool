...
// Check whether pickup and destination are after current Tesla position
...
const isFutureStop = (
  ...
  const pickupIndex = currentRoute.indexOf(pickupLocation);

  const destinationIndex = currentRoute.indexOf(destinationLocation);
...
const generatePossibleRoutes = (
  ...
  const activeRoute = currentRoute.slice(currentIndex);

  for (let pickupIndex = 0; pickupIndex <= activeRoute.length; pickupIndex++) {
    const routeWithPickup = [
      ...currentRoute.slice(0, currentIndex),
      ...activeRoute.slice(0, pickupIndex),
      pickupLocation,
      ...activeRoute.slice(pickupIndex),
    ];

    for (
      let destinationIndex = pickupIndex + 1;
      destinationIndex <= activeRoute.length + 1;
      destinationIndex++
    ) {
      const routeWithDestination = [
        ...routeWithPickup.slice(0, currentIndex + destinationIndex),
        destinationLocation,
        ...routeWithPickup.slice(currentIndex + destinationIndex),
      ];
