...
const MIGRATION_UP_DIR = path.join(__dirname, "migrations/up");

const MIGRATION_DOWN_DIR = path.join(__dirname, "migrations/down");
...
async function executeMigration(file, directory) {
...
async function migrateUp() {
  ...
  const files = await getMigrationFiles(MIGRATION_UP_DIR);
  ...
  for (const file of files) {
    ...
    await executeMigration(file, MIGRATION_UP_DIR);
  }
...
async function migrateDown() {
  ...
  const downFile = latest.replace(".sql", "_down.sql");

  const downPath = path.join(MIGRATION_DOWN_DIR, downFile);
